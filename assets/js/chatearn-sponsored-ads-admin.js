/* ChatEarn Sponsored Ads Manager v1.0.0 */
(() => {
  'use strict';
  ...
  const VERSION='1.0.0';
  const deletedKey='ce_sponsored_ads_deleted_keys_v1';
  ...
  function buildPanel(){
    ...
    panel.innerHTML=`<div class="admin-status-banner"><b>Sponsored Ads:</b> Add each ad once, then edit, archive, unarchive, preview or delete it here. Active ads rotate automatically.</div>
    <div class="admin-section-title">Create or edit sponsored ad</div>
    <form id="ceSponsoredAdForm" class="ce-v42-admin-form">
      <input id="ceAdHeadline" class="wide" placeholder="Ad headline" required maxlength="90">
      <textarea id="ceAdDescription" class="wide" placeholder="Short description" required maxlength="220"></textarea>
      <input id="ceAdCta" placeholder="Button text, e.g. Open Now" required maxlength="40">
      <input id="ceAdUrl" class="wide" type="url" placeholder="Sponsored link: https://..." required>
      <select id="ceAdAudience"><option value="all">All users</option><option value="new">New users</option><option value="returning">Returning users</option></select>
      <input id="ceAdOrder" type="number" min="1" max="100" value="10" title="Lower numbers show first">
      <div class="wide" style="display:grid;gap:8px;padding:12px;border:1px solid var(--line);border-radius:12px"><b>Where should this ad appear?</b>
        <label><input type="checkbox" data-ce-ad-placement value="chat_banner" checked> Bottom chat banner</label>
        <label><input type="checkbox" data-ce-ad-placement value="chat_half_screen"> Half-screen chat pop-up</label>
        <label><input type="checkbox" data-ce-ad-placement value="chat_native"> Sponsored card inside chat</label>
        <label><input type="checkbox" data-ce-ad-placement value="dashboard"> Dashboard</label>
        <label><input type="checkbox" data-ce-ad-placement value="earnings"> Earnings page</label>
      </div>
      <button id="ceAdSaveBtn" class="admin-btn primary wide" type="submit">Save Sponsored Ad</button>
      <button id="ceAdCancelEdit" class="admin-btn wide" type="button" style="display:none">Cancel Editing</button>
    </form>
    <div class="admin-toolbar"><div class="admin-section-title">Active sponsored ads</div><button class="admin-btn" id="ceAdRefresh">Refresh</button></div>
    <div id="ceAdActiveList" class="admin-list"></div>
    <div class="admin-section-title">Archived sponsored ads</div><div id="ceAdArchivedList" class="admin-list"></div>
    <div id="ceAdPreviewModal" style="display:none;position:fixed;inset:0;z-index:10050;background:rgba(0,0,0,.72);align-items:center;justify-content:center;padding:20px"><div style="width:min(430px,100%);background:var(--card);border:1px solid var(--line);border-radius:18px;padding:20px"><div id="ceAdPreviewBody"></div><button class="admin-btn wide" id="ceAdPreviewClose" type="button" style="margin-top:14px">Close Preview</button></div></div>`;
  ...
  function boot(){buildPanel();const tab=[...document.querySelectorAll('#adminTabs .admin-tab')].find(x=>/sponsored ads/i.test(x.textContent||''));tab?.addEventListener('click',()=>setTimeout(loadManager,80));}
