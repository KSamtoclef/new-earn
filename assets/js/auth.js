(() => {
  ...
  async function boot() {
    if (!client) return notify('Secure connection failed to initialize.', true);
    ...
    if (data.session?.user) await enterAccount(data.session.user); else show('landing');
    client.auth.onAuthStateChange(event => { if (event === 'SIGNED_OUT') show('landing'); });
  }
