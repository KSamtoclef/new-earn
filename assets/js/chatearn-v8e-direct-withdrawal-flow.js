/* ChatEarn V8E.6 — quiet, persistent direct withdrawal journey. */
(() => {
  'use strict';
  if (window.__CHAT_EARN_V8E6_DIRECT_WITHDRAWAL__) return;
  window.__CHAT_EARN_V8E6_DIRECT_WITHDRAWAL__ = true;

  const VERSION='8E.6',DRAFT_KEY='ce_withdrawal_draft_v8e',FLOW_KEY='ce_withdrawal_flow_v8e',IDEMPOTENCY_KEY='ce_direct_withdrawal_key';
  ...
  const technical=t=>/public\.chatearn_|schema cache|could not find the function|permission denied for function|withdrawal service is refreshing/i.test(String(t||''));
  function removeTechnicalNotices(root=document){
    root.querySelectorAll?.('.toast,.ce-toast,[role="alert"],.admin-status-banner,.error-toast,.toast-message').forEach(el=>{if(technical(el.textContent))el.remove()});
    const portal=byId('withdrawalPortalStatus');if(portal&&technical(portal.textContent))portal.style.display='none';
  }
  function patchToast(){
    if(window.__CE_QUIET_WITHDRAWAL_TOAST__)return;
    window.__CE_QUIET_WITHDRAWAL_TOAST__=true;
    const original=window.showToast;
    if(typeof original==='function')window.showToast=function(message,...rest){if(technical(message))return;return original.call(this,message,...rest)};
  }
  function cleanOldUi(){
    ...
    removeTechnicalNotices();
  }
  function visibleAmount(){
    const stateAmount=Number(window.ChatEarnWithdrawalV5?.getState?.()?.portal?.wallet?.available_balance||0);if(stateAmount>0)return stateAmount;
    try{if(Number(totalBalance)>0)return Number(totalBalance)}catch(_){}
    const text=document.querySelector('#withdraw .wd-amount-display,#withdraw .amount-display,#withdraw .wd-amount')?.textContent||'';return Number(text.replace(/[^0-9]/g,''))||0;
  }
  function openNextStage(result){
    const withdrawalId=result?.withdrawal_id||result?.id||null;
    saveFlow({withdrawal_id:withdrawalId,stage:'sharing_required'});
  ...
  function ensureDirectForm(){
    ...
    f.onsubmit=async e=>{
      e.preventDefault();removeTechnicalNotices();
      ...
      persist();btn.disabled=true;btn.textContent='Placing Withdrawal…';status.textContent='Securing your request…';
      ...
      try{
        ...
        status.textContent='Withdrawal placed successfully.';setTimeout(()=>openNextStage(r),120);
      }catch(err){console.warn('[ChatEarn] withdrawal request failed',err);status.textContent='Please tap the button again.';}
  ...
  function boot(){patchToast();cleanOldUi();let s='';try{s=currentScreen||''}catch(_){}if(s==='withdraw')ensureDirectForm();if(s==='processing')showContinue();resume()}
  ...
  console.info(`[ChatEarn] Quiet withdrawal flow ${VERSION} loaded`);
})();