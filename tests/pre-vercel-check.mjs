...
for (const path of requiredFiles) {
  expect(fs.existsSync(path), `${path} exists`);
}
...
expect(index.includes('./assets/js/config.js') && index.includes('./assets/js/app.js'), 'index loads only canonical config and app entrypoints');
...
const allActive = [config, app, auth, chat, rewards, withdrawal, content, admin, stabilization].join('\n');
...
for (const id of ['landing','register','login','loading','dashboard','toast']) {
  expect(index.includes(`id="${id}"`), `index contains #${id}`);
}
...
expect(!withdrawal.includes("state.shares += 1"), 'legacy immediate share increment removed or overridden');
